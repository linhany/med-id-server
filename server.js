...
app.post('/doctor/:id/authorize', (req, res) => {
  ...
  Doctor.findById(req.params.id, function(err, doctor) {
    ...
    if (!isValid) return res.status(404).send("ERROR: Hash of healthrecord does not equate to hash on blockchain.");
    console.log("Verified healthrecord with blockchain.")
    ...
    if (authorizedHRArr.indexOf(patientID) === -1) { // if not already authorized
      ...
      console.log("Healthrecord already authorized.")
    }
